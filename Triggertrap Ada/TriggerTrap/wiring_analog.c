...
int analogRead12(uint8_t pin)
{
    ...
    if ( pin < 8 ) {
        ...
        pin -= 8;
#else		
    if ( pin < 8 ) {		//TriggerTrap board is the other configuration to Arduino
        adc = &ADCB;
    } else if ( pin < 16 ) {
        adc = &ADCA;
        pin -= 8;
#endif //_USE_ARDUINO_ORIGINAL_CODE	
...
#else
	pin = adcToChannel((pin - 4));	//Mod to work around hardware differences
#endif //_USE_ARDUINO_ORIGINAL_CODE
